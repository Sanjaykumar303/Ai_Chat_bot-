...
# mode: a confidently-stated wrong/fabricated answer from a database
# result that only *looks* valid (a zero from an unmatched filter, or a
# "today" query that never actually filtered to today) - see
# db_query_service.py's own module comment for the real incidents that
# led to each one.
#
# Moved here from tests/test_chat_service_guards.py (formerly
# chat_service.py's own functions) when the guards themselves moved to
# db_query_service.py, so they can pre-empt generation - checked on the
# raw rows/SQL BEFORE any prose is generated - rather than replacing
# already-generated (and, once streaming, already-shown-to-the-user)
# text afterward.
